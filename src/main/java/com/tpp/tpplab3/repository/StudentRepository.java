...
public class StudentRepository {
    ...
    public List<Student> findAll() {
        ...
        try (Connection conn = DriverManager.getConnection(url, user, password);
            ...
            while (rs.next()) {
                result.add(new Student(rs.getLong("id"), rs.getString("first_name"), rs.getString("last_name"), rs.getInt("group_id")));
            }
    ...
    // КУРСИ - ВИПРАВЛЕНО (course_name з твого pgAdmin)
    ...
    // ОЦІНКИ - ВИПРАВЛЕНО (grade з твого pgAdmin)
    public List<Grade> findAllGrades() {
        ...
        try (Connection conn = DriverManager.getConnection(url, user, password);
             ...
             ResultSet rs = stmt.executeQuery(sql)) {
            while (rs.next()) {
                grades.add(new Grade(rs.getInt("id"), rs.getLong("student_id"), rs.getInt("course_id"), rs.getInt("grade")));
            }
        } catch (SQLException e) { e.printStackTrace(); }
        return grades;
    }
